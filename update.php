...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($name_err) &&empty($username) && empty($sex_err) && empty($address_err) && empty($phone_err) && empty($salary_err)){
        // Prepare an update statement
        $sql = "UPDATE employees SET name=?, username=?, sex=?, address=?, phone=?, salary=?, password=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssssii", $param_name, $param_username, $sex_username, $param_address, $param_phone, $param_salary, $param_id);
